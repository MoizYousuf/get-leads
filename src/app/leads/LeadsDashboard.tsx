    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {/* Mobile Search Results Card View (Mobile only) */}
          <div className="md:hidden space-y-3.5 px-1">
            {paginatedLeads.map((lead) => {
              ...
              return (
                <div
                  key={lead.id}
                  className={`bg-slate-900/40 border rounded-2xl p-4 space-y-3.5 transition backdrop-blur-md ${
                    isSelected ? "border-sky-500/50 bg-sky-500/[0.02]" : "border-slate-800/60"
                  }`}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      {lead.email ? (
                        ...
                        <div className="w-4.5 h-4.5 rounded border border-slate-850 bg-slate-950/60 opacity-30 cursor-not-allowed flex items-center justify-center">
                          <span className="w-1.5 h-1.5 bg-slate-700 rounded-full" />
                        </div>
                      )}
                      <div>
                    ...
                    {(lead as any).isImported && (
                      <span className="text-[8px] font-black uppercase bg-indigo-500/15 border border-indigo-500/30 text-indigo-400 px-2 py-0.5 rounded shrink-0">
                        Saved to CRM
                      </span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-400 border-t border-b border-slate-900 py-2.5">
                    <div>
                      ...
                      {lead.email ? (
                        <button
                          ...
                          className="truncate block text-sky-400 font-mono font-bold text-left hover:underline max-w-[120px]"
                        >
                          {lead.email}
                        </button>
                      ) : (
                    ...
                    <div>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-slate-300 hover:text-sky-400 flex items-center gap-1 font-semibold truncate block"
                      >
                    ...
                    <div>
                      ...
                      {lead.website ? (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-sky-400 hover:text-sky-350 flex items-center gap-1 font-semibold truncate block"
                        >
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    ...
                    {!lead.email && (
                      <button
                        ...
                        onClick={() => handleEnrichLead(lead.id)}
                        disabled={enrichingIds.has(lead.id)}
                        className="flex-1 py-2 bg-slate-900 border border-slate-800 hover:bg-slate-850 hover:text-slate-200 rounded-xl text-xs font-bold text-slate-300 transition flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                      >
                        {enrichingIds.has(lead.id) ? (
                          <span className="w-3.5 h-3.5 border-2 border-sky-400/20 border-t-sky-400 rounded-full animate-spin" />
                        ) : (
                    ...
                    {!(lead as any).isImported ? (
                      ...
                      <div className="flex-1 py-2 bg-slate-950 text-slate-650 border border-slate-900 rounded-xl text-center text-xs font-bold select-none">
                        Saved to CRM
                      </div>
                    )}
                  </div>
